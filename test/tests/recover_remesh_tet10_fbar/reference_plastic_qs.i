...
#   - PresetDisplacement ypull BC -> FunctionDirichletBC
...
# Pair with restart_plastic_qs.i.
#
...
[AuxVariables]
  ...
  # Plastic QS: accel_*, vel_* AuxVariables removed (no Newmark).
[]
...
[Kernels]
  # Plastic QS: ADInertialForce x/y/z removed.  Stress divergence uses the
  # plain ADStressDivergenceTensors -- no HHT-alpha damping.
  [x]
...
[RecoverVariables]
  [rec]
    # stretch_tensor (= U raw) and stretch_tensor_fbar (= U_bar) are both
...
[BCs]
  ...
  []
  [ypull]
    # Plastic QS: FunctionDirichletBC (no Newmark velocity/accel state).
