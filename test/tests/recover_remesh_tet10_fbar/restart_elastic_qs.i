...
#
# Strip relative to restart_elastic.i:
...
#   - PresetDisplacement BC -> FunctionDirichletBC
#   - hht_alpha / newmark_beta / newmark_gamma constants removed
# Pair with reference_elastic_qs.i.
#
...
# BC constraint pattern (CLI-overridable for the BC sweep).  Must match the
# values passed to the reference's run.  Defaults reproduce the original
# heavily-constrained setup.
xfix_bnd = 'left top'
yfix_bnd = 'bottom'
zfix_bnd = 'front back'
...
[UserObjects]
  [epsol]
    ...
    mesh = ${recover_file}
    # Elastic QS: T dropped (no thermal) and accel_* / vel_* dropped (no inertia).
    system_variables = 'd d_old d_corr'
...
[AuxVariables]
  ...
  [d_corr]
    ...
    # []
  []
  # Elastic QS: accel_* and vel_* AuxVariables removed (no Newmark).
[]
...
[Kernels]
  # Elastic QS: inertia kernels removed.  Stress divergence uses the plain
  # ADStressDivergenceTensors -- no HHT-alpha, no dynamic-recover special
  # first-step branch.  The recovered F_bar (from defgrad/recover=true) still
  # enters via _stress and ComputeLargeDeformationStress.
  [x]
...
[Materials]
  ...
  []
  # Elastic QS: ADStrainAdjustedDensityCustom removed (nothing reads adj_density
...
[BCs]
  [xfix]
    type = DirichletBC
    variable = disp_x
    # CLI-overridable for the BC sweep -- must match the reference's xfix_bnd
    # so the recovered state stays in equilibrium with the same constraints.
    boundary = '${xfix_bnd}'
    value = 0
    preset = false
  []
