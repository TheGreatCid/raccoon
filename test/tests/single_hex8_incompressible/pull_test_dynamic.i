...
# loading alone.
#   target_uy(top) = pull_amount * (t/end_time_for_ramp) * X0
#                  + pulse_amplitude * exp(-((t - pulse_center)/pulse_width)^2)
# Default pulse_amplitude=0.01 = 5% of pull_amount.  Set pulse_amplitude=0 to
# disable.  pulse_center kept early (0.05) so the pulse is fully launched
# before any reasonable dump_time, and pulse_width chosen on the order of one
# longitudinal transit time at high nu so the wave is sharp.
# Pulse is split into two components so we can control bending kinematics and
# uniform-vertical (incompressibility-stimulus) content independently:
#   pulse_amplitude_bend     : x-scaled, adds to bending kinematics (visible wave)
#   pulse_amplitude_compress : x-independent, adds uniform vertical motion of the
#                              entire top face (drives F-bar locking work because
#                              the body must dilate to accommodate it while
#                              staying incompressible)
...
# Quadratic startup window for the bending ramp.  Without it, ADMatchedValueBC
# enforces disp_y(top, t=dt) = pull/eft*dt*X0 at step 1 while the vel_y/accel_y
# auxvars are still at their default zero IC; NewmarkAccelAux's TIMESTEP_END
# back-computation then produces a ~1/(beta*dt) acceleration spike on the BC
# nodes that propagates through the element mass matrix and pollutes the
# interior dynamics.  Replacing the linear ramp with a piecewise quadratic-
# then-linear profile gives a smooth velocity ramp-up from rest (f'(0)=0,
# f''(0) = pull/(eft*ramp_time)*X0 finite) so NewmarkAccelAux/VelAux track
# the analytical kinematics correctly from t=0.
# Asymptote for t > ramp_time is pull/eft*(t - ramp_time/2)*X0, which differs
# from the original linear ramp by a constant offset that cancels in the
# restart's incremental BC formula -- so the restart side stays unchanged as
# long as start_time > ramp_time.
ramp_time = 0.1
...
[Functions]
  [pull_func]
    ...
    #  expression = 'if(t <= ramp_time, pull_amount / (2*end_time_for_ramp*ramp_time) * t*t * x, pull_amount / end_time_for_ramp * (t - ramp_time/2) * x)'
...
[BCs]
  ...
  [ypull]
    ...
    # PresetDisplacement (see src/bcs/PresetDisplacementSpatial.C).  Enforces
    # the BC through Newmark's forward update formula
    #   disp_n+1 = u_old + dt*v_old + ((0.5-beta)*a_old + beta*a_new)*dt^2
    # where `a_new` is the numerical f''(t) of `function` at the current
    # NODE'S position (not the default-constructed (0,0,0) the stock
    # PresetDisplacement uses), so spatially-varying loads work correctly.
    #
    # Why we need this and not ADMatchedValueBC: that BC enforces disp = target_uy
    # directly, leaving NewmarkAccelAux at TIMESTEP_END to back-compute a_n+1
    # via a_n+1 = (Delta_u - dt*v_old - (0.5-beta)*dt^2*a_old) / (beta*dt^2).
    # The 1/(beta*dt) factor on (c' - v_old) means any tiny mismatch between
    # v_old (Aux IC default = 0) and the BC's analytical velocity blows up the
    # acceleration as dt shrinks -- spectrum observed in practice as wildly
    # oscillating accels that get larger at smaller dt.  PresetDisplacement-
    # style BCs avoid this by running the Newmark update FORWARD.
...
[Materials]
  ...
  # ----- Per-QP stress.trace() diagnostic ----------------------------------
  # F-bar's variational identity requires sigma.trace() to be EXACTLY uniform
  # within each element (= 3 * K * log(J_avg) / J_avg).  In floating point,
  # the constitutive evaluation can give slightly different traces per QP
  # because the per-QP F differs going in.  If that per-QP roundoff scales
  # with K (i.e., max(sigma.trace()) - min(sigma.trace()) over the single
  # element grows as nu -> 0.5), then the kernel's B-bar correction term
  #     (sigma.trace()/3) * (avg_grad_test - grad_test_component)
  # no longer integrates to zero at the element level even at the converged
  # state, and Newton has to balance a K-amplified spurious force.  That is
  # the candidate mechanism for the K-scaled first-step error in the dynamic
  # restart even with hht_alpha = 0.
  [stress_trace]
    type = ADRankTwoInvariant
    rank_two_tensor = stress
    invariant = FirstInvariant
    property_name = stress_trace
  []
...
[Postprocessors]
  [psie_active_int]
    ...
    use_displaced_mesh = true
  []
  # ----- Per-QP stress.trace() spread diagnostic ---------------------------
  # In a F-bar consistent formulation sigma.trace() should be uniform inside
  # each element.  If stress_trace_spread > 0 and scales with K (= grows
  # rapidly as nu -> 0.5), the constitutive's per-QP roundoff in the trace
  # is the K-amplification channel we suspect.
  [stress_trace_max]
    type = ADElementExtremeMaterialProperty
    mat_prop = stress_trace
    value_type = max
  []
  [stress_trace_min]
    type = ADElementExtremeMaterialProperty
  ...
  [stress_trace_spread]
    ...
    value1 = stress_trace_max
    value2 = stress_trace_min
  []
  [J_F_int]
    type = ADElementIntegralMaterialProperty
    mat_prop = J_F
