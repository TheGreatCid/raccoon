# Single-TET10 incompressible BENDING test (PLASTIC_QS variant -- QS + J2).
#
# Quasi-static counterpart of pull_test_plastic.i.  Adds J2 plasticity with
# PowerLawHardening on top of the elastic QS reference (pull_test.i) so the
# body yields under the bending load WITHOUT inertia.  be_bar +
# effective_plastic_strain are dumped through RecoverVariables so the restart
# leg can pick up the isochoric plastic state.  Pairs with
# pull_test_plastic_qs_restart.i.
#
# Original docstring (from the elastic QS variant) below:
...
# PowerLawHardening parameters (must match pull_test_plastic.i).  Tuned well
# below the elastic stress level under the bending pull so plastic flow is
# actually triggered.
yield_stress = 0.03
...
[Functions]
  [pull_func]
    ...
    type = ParsedFunction
    expression = 'pull_amount * (t/end_time) * x'
    symbol_names = 'pull_amount end_time'
    symbol_values = '${pull_amount} ${end_time}'
  []
[]
...
[Materials]
  ...
  []
  [JC]
    # PowerLawHardening kept under the name `JC` for parity with the
    # recover_remesh_tet10_fbar test naming.
