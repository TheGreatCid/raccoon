...
#include "JohnsonCookHardeningMod.h"

registerMooseObject("raccoonApp", JohnsonCookHardeningMod);

InputParameters
JohnsonCookHardeningMod::validParams()
{
...
JohnsonCookHardeningMod::JohnsonCookHardeningMod(const InputParameters & parameters)
...
JohnsonCookHardeningMod::temperatureDependence()
{
  return 1 - std::pow((_T[_qp] - _T0) / (_Tm - _T0), _m);
}

ADReal
JohnsonCookHardeningMod::initialGuess(const ADReal & effective_trial_stress)
{
...
ADReal
JohnsonCookHardeningMod::plasticEnergy(const ADReal & ep, const unsigned int derivative)
{
  if (derivative == 0)
  {
    _psip_active[_qp] = -(-1 + _tqf) * _sigma_0[_qp] *
                        (_A[_qp] + _A[_qp] * _n + _B * std::pow(ep, _n)) * temperatureDependence();
  ...
  if (derivative == 1)
  {
    return _gp[_qp] * (1 - _tqf) * _sigma_0[_qp] * (_B * std::pow(ep, _n) + _A[_qp]) *
           temperatureDependence();
  }
  if (derivative == 2)
  {
    return _gp[_qp] * (1 - _tqf) * _B * _n * std::pow(ep, _n - 1) * temperatureDependence();
  }
  mooseError(name(), "internal error: unsupported derivative order.");
}

ADReal
JohnsonCookHardeningMod::plasticDissipation(const ADReal & delta_ep,
                                            const ADReal & ep,
                                            const unsigned int derivative)
{
  ...
  if (derivative == 0)
  {
    result += delta_ep / _dt * (_A[_qp] * (_tqf - _C) + _B * _tqf * (std::pow(ep, _n)));
    if (_t_step > 0 && delta_ep > libMesh::TOLERANCE * libMesh::TOLERANCE)
      result += (_A[_qp] * _C * std::log(delta_ep / _dt / _epdot0)) * delta_ep / _dt;
  }

  if (derivative == 1)
  {
    result += _tqf * (_B * std::pow(ep, _n) + _A[_qp]);
    if (_t_step > 0 && delta_ep > libMesh::TOLERANCE * libMesh::TOLERANCE)
      result += _A[_qp] * _C * std::log(delta_ep / _dt / _epdot0);
  }

  if (derivative == 2)
  {
    result += _tqf * _B * _n * (std::pow(ep, _n - 1));
  }
...
JohnsonCookHardeningMod::thermalConjugate(const ADReal & ep)
{

  return _T[_qp] * _gp[_qp] * (1 - _tqf) * _sigma_0[_qp] * (_B * std::pow(ep, _n) + _A[_qp]) *
