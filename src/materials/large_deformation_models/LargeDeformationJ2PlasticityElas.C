...
#include "LargeDeformationJ2PlasticityElas.h"
#include "RaccoonUtils.h"

registerMooseObject("raccoonApp", LargeDeformationJ2PlasticityElas);

InputParameters
LargeDeformationJ2PlasticityElas::validParams()
{
...
LargeDeformationJ2PlasticityElas::LargeDeformationJ2PlasticityElas(
...
void
LargeDeformationJ2PlasticityElas::updateState(ADRankTwoTensor & stress, ADRankTwoTensor & Fe)
{
...
Real
LargeDeformationJ2PlasticityElas::computeReferenceResidual(const ADReal & effective_trial_stress,
...
ADReal
LargeDeformationJ2PlasticityElas::computeResidual(const ADReal & effective_trial_stress,
                                                  const ADReal & delta_ep)
{
  ...
  return effective_trial_stress -
         ...
         _hardening_model->plasticDissipation(delta_ep, ep, 1) - _psic_model->elasticEnergy(ep, 1);
}

ADReal
LargeDeformationJ2PlasticityElas::computeDerivative(const ADReal & /*effective_trial_stress*/,
                                                    const ADReal & delta_ep)
{
  ADReal ep = _ep_old[_qp] + delta_ep;
  if (ep == 0)
  ...
  return -_elasticity_model->computeMandelStress(_Np[_qp], /*plasticity_update = */ true)
         ...
         _hardening_model->plasticDissipation(delta_ep, ep, 2) - _psic_model->elasticEnergy(ep, 2);
}
