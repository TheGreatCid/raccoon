...
//   params.addRequiredParam<Real>("threshold", "The threshold for an element to be in the
//   nodeset."); params.addParam<BoundaryName>(
...
//   const std::vector<BoundaryID> boundary_ids = mesh.getBoundaryIDs({{_moving_boundary_name}},
//   true); mooseAssert(boundary_ids.size() == 1, "Expect exactly one boundary ID.");
