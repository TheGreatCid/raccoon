...
SolutionICArr::validParams()
{
  ...
  params.addParam<std::vector<std::string>>(
      "variables", "The name of the variables in the file that are to be extracted");
...
SolutionICArr::SolutionICArr(const InputParameters & parameters)
  : Material(parameters),
    _solution_object(getUserObject<SolutionUserObject>("solution_uo")),
    _system_variables(getParam<std::vector<std::string>>("variables")),
...
Real
SolutionICArr::returnval(const int & j, const Point & p)
{
  return _solution_object.pointValue(0., p, _system_variables[j], &_exo_block_ids);
}
